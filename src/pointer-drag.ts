...
function isHtmlElement(target: EventTarget | null): target is HTMLElement {
  return !!target && (target as Node).instanceOf?.(HTMLElement) === true;
}
...
function markShift(el: HTMLElement, opacity?: string): void {
  ...
  if (opacity !== undefined) el.setCssStyles({ opacity });
}

function createLightGhost(source: HTMLElement): HTMLElement {
  const rect = source.getBoundingClientRect();
  const ghost = createDiv({ cls: "jnr-drag-ghost jnr-drag-ghost-lite" });
  ghost.setCssStyles({
    width: `${rect.width}px`,
