...
export const USAGE_GUIDE_PATH = "纪念日 插件使用说明.md";
export const USAGE_GUIDE_VERSION = "2026-08-28-jinianri-guide-v14";

const GUIDE_VERSION_RE = /<!--\s*jnr-guide-version:([^>]+)\s*-->/;

function buildGuideFileContent(): string {
  return `${USAGE_GUIDE_CONTENT.trim()}\n\n<!-- jnr-guide-version:${USAGE_GUIDE_VERSION} -->`;
}

function extractGuideVersion(content: string): string | null {
  const m = content.match(GUIDE_VERSION_RE);
  return m ? m[1].trim() : null;
}

/** 正文不含一级标题，避免与 Obsidian 文件名标题重复 */
export const USAGE_GUIDE_CONTENT = `纪念日插件用于记录生日、恋爱、婚姻等重要日期，自动计算「已过时长」与「距离下次还有几天」，并支持三档全局提醒与 iCal 导出。
...
export async function ensureWelcomeGuideFile(app: App): Promise<void> {
  try {
...
/** 在新标签页打开完整使用说明（主编辑区，非 Modal） */
export async function openUsageGuideInNewTab(app: App): Promise<void> {
...
/** 手动打开完整说明文档 */
export async function openWelcomeGuideOnce(app: App, options: OpenGuideOptions = {}): Promise<void> {
