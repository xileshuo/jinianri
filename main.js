...
function renderLifeOsLicenseSettingsPanel(panel, config) {
  ...
  const heading = new import_obsidian2.Setting(card).setName("\u6388\u6743\u6FC0\u6D3B").setHeading();
...
function renderLifeOsAboutPanel(panel, plugin, options = {}) {
  ...
  new import_obsidian2.Setting(helpBlock).setName("\u6587\u6863").setHeading();
  ...
  new import_obsidian2.Setting(authorBlock).setName("\u4F5C\u8005").setHeading();
  ...
  new import_obsidian2.Setting(worksBlock).setName("\u6240\u6709\u4F5C\u54C1").setHeading();
  ...
  LIFEOS_PLUGIN_CATALOG.forEach((item) => {
    ...
    if (item.id === selfId) {
      ...
      btn.onclick = () => {
        ...
        else new import_obsidian2.Notice(`\u8BF7\u5148\u5728 Obsidian \u8BBE\u7F6E \u2192 \u7B2C\u4E09\u65B9\u63D2\u4EF6 \u4E2D\u542F\u7528 ${item.name}`);
      };
    }
  });
}
...
var JinianriSettingTab = class extends import_obsidian14.PluginSettingTab {
  ...
  display() {
    ...
    if (!isMobile) {
      new import_obsidian14.Setting(containerEl).setName(formatPluginSettingsTitle("\u7EAA\u5FF5\u65E5 \u914D\u7F6E", getEditionLabel())).setHeading();
    }
  ...
  createBlock(parent, title, desc) {
    ...
    const heading = new import_obsidian14.Setting(block).setName(title).setHeading();
    if (desc) heading.setDesc(desc);
    return block;
  }
  addSubgroupTitle(block, title) {
    new import_obsidian14.Setting(block).setName(title).setHeading().setClass("jnr-settings-subgroup");
  }
  ...
  renderOnboarding(parent) {
    ...
    new import_obsidian14.Setting(box).setName("\u5F00\u59CB\u4F7F\u7528").setHeading();
  ...
  renderEventsPanel(panel) {
    ...
    for (const groupId of this.plugin.getGroupOrder()) {
      ...
      new import_obsidian14.Setting(block).setName(def.label).setHeading();
