        ...
        var _fromYmdHms = function(y, m, d, hour, minute, second) {
          ...
          return {
            ...
            nextDay: function(days) {
              ...
              if (days > 0) {
                d2 += days;
                var daysInMonth2 = SolarUtil.getDaysOfMonth(y2, m2);
                while (d2 > daysInMonth2) {
                  ...
                  daysInMonth2 = SolarUtil.getDaysOfMonth(y2, m2);
                }
              } else if (days < 0) {
                while (d2 + days <= 0) {
                  m2--;
...
var require_lunar_javascript = __commonJS({
  "node_modules/lunar-javascript/index.js"(exports, module2) {
    var { Solar: Solar3, Lunar: Lunar2, Foto, Tao, NineStar, EightChar, SolarWeek, SolarMonth, SolarSeason, SolarHalfYear, SolarYear, LunarMonth, LunarYear, LunarTime, ShouXingUtil, SolarUtil, LunarUtil, FotoUtil, TaoUtil, HolidayUtil: HolidayUtil2, NineStarUtil, I18n } = require_lunar();
    module2.exports = {
      Solar: Solar3,
      Lunar: Lunar2,
      Foto,
...
function normalizeEvent(raw) {
  ...
  return {
    ...
    type: raw.type === "lunar" ? "lunar" : "solar",
...
function findNextSolarForLunarMD(lunarMonth, lunarDay, minDate) {
  const start = startOfDay(minDate);
  for (let i = 0; i < 400; i++) {
    const candidate = addDays(start, i);
    const lunar = solarFromDate(candidate).getLunar();
    const month = Math.abs(lunar.getMonth());
    if (month === lunarMonth && lunar.getDay() === lunarDay) {
      return candidate;
    }
  }
...
function getNextOccurrence(event, from = /* @__PURE__ */ new Date()) {
  ...
  if (event.type === "lunar") {
    return findNextSolarForLunarMD(origin.month, origin.day, today);
  }
  let candidate = new Date(today.getFullYear(), origin.month - 1, origin.day);
  if (candidate < today) {
    candidate = new Date(today.getFullYear() + 1, origin.month - 1, origin.day);
  }
  return candidate;
}
function getOriginSolarDate(event) {
  ...
  if (event.type === "lunar") {
    const lunar = import_lunar_javascript.Lunar.fromYmd(year, month, day);
...
function computeYearlyAnniversary(event, now = /* @__PURE__ */ new Date()) {
  ...
  if (event.type === "solar" && anniversaryThisYear > today) {
    yearsPassed -= 1;
  }
  if (event.type === "lunar") {
...
function getLastOccurrence(event, now = /* @__PURE__ */ new Date()) {
  ...
  while (candidate >= origin) {
    const next = startOfDay(getNextOccurrence(event, candidate));
...
function markShift(el, opacity) {
  ...
  if (opacity !== void 0) el.style.opacity = opacity;
}
...
function getVaultScopedStorageKey(app2, suffix) {
  var _a, _b;
  const vaultName = ((_b = (_a = app2.vault) == null ? void 0 : _a.getName) == null ? void 0 : _b.call(_a)) || "UnknownVault";
  return `jinianri:${vaultName}:${suffix}`;
}
function ensureTrialStarted(app2, settings, force = false) {
  if (!isTrialEdition() || syncLicenseState(app2, settings)) return;
  if (!force && !settings.trialWelcomeSeen) return;
  const storageKey = getVaultScopedStorageKey(app2, "trialStartedAt");
...
function getTrialRemainingMs(app2, settings) {
  if (!isTrialEdition() || syncLicenseState(app2, settings)) return 0;
  ensureTrialStarted(app2, settings);
...
function isTrialActive(app2, settings) {
  return isTrialEdition() && getTrialRemainingMs(app2, settings) > 0;
}
function isPluginAccessAllowed(app2, settings) {
  if (!isLicenseRequired()) return true;
  if (syncLicenseState(app2, settings)) return true;
  return isTrialActive(app2, settings);
}
...
function getDeviceFingerprint(app2) {
  var _a, _b;
  if (app2.appId) return "JNR-" + hashStringToHex(String(app2.appId));
  const vaultName = ((_b = (_a = app2.vault) == null ? void 0 : _a.getName) == null ? void 0 : _b.call(_a)) || "UnknownVault";
...
function getExpectedLicenseKeys(app2) {
  var _a, _b;
  const keys = /* @__PURE__ */ new Set();
  keys.add(computeExpectedLicenseKeyFromFingerprint(getDeviceFingerprint(app2)));
  const vaultName = ((_b = (_a = app2.vault) == null ? void 0 : _a.getName) == null ? void 0 : _b.call(_a)) || "UnknownVault";
  keys.add(computeExpectedLicenseKeyFromFingerprint("JNR-" + hashStringToHex(String(vaultName))));
  return keys;
}
function isLicenseValid(app2, key) {
  if (!key || !String(key).trim()) return false;
  const normalized = String(key).trim().toUpperCase();
  for (const expected of getExpectedLicenseKeys(app2)) {
    if (normalized === expected) return true;
  }
  return false;
}
function syncLicenseState(app2, settings) {
  if (!isLicenseRequired()) {
    settings.licenseActivated = true;
    return true;
  }
  const ok = isLicenseValid(app2, settings.licenseKey);
  settings.licenseActivated = ok;
  return ok;
}
function isPluginLicensed(app2, settings) {
  return isPluginAccessAllowed(app2, settings);
}
...
function renderEditionBadge(parent) {
  ...
  parent.createSpan({
    ...
    text: label,
    attr: {
      title: isTrialEdition() ? `\u4F53\u9A8C\u7248 \xB7 ${getTrialHoursLabel()}\u5168\u529F\u80FD\u8BD5\u7528` : isLicenseEnforced() ? "\u516C\u7248 \xB7 \u9700\u6FC0\u6D3B\u540E\u4F7F\u7528\u5168\u90E8\u529F\u80FD" : "\u4E2A\u4EBA\u7248 \xB7 \u5DF2\u5185\u7F6E\u6388\u6743\uFF0C\u53EF\u76F4\u63A5\u4F7F\u7528"
    }
  });
}

// src/lifeos-ui-shared.ts
var LIFEOS_MOBILE_TOP_INSET_PX = 41;
var LIFEOS_RELEASE = "2026.08.10";
var LIFEOS_UI_STYLE_ID = "lifeos-ui-shared-styles-v9";
...
.lifeos-about-work-item{padding:12px 14px;border-radius:12px;border:1px solid var(--background-modifier-border);background:var(--background-secondary)}
@media (min-width:640px){.lifeos-about-works{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;align-items:stretch}.lifeos-about-work-item{height:100%;box-sizing:border-box}}
...
.lifeos-about-work-actions button{border-radius:8px;padding:4px 10px;font-size:11px;font-weight:600;cursor:pointer;border:1px solid var(--background-modifier-border);background:var(--background-modifier-hover);color:var(--text-muted)}
...
function openLifeOsPluginSettings(app2, pluginId) {
  if (!(app2 == null ? void 0 : app2.setting)) return;
  const openTab = () => {
    var _a, _b;
    try {
      if (typeof app2.setting.openTabById === "function") {
        app2.setting.openTabById(pluginId);
        return true;
      }
      const tab = (_b = (_a = app2.setting.pluginTabs) == null ? void 0 : _a.find) == null ? void 0 : _b.call(_a, (t) => t.id === pluginId);
      if (tab && typeof app2.setting.openTab === "function") {
        app2.setting.openTab(tab);
        return true;
      }
    } catch (e) {
    }
  ...
  if (openTab()) return;
  window.setTimeout(() => {
    if (!openTab()) new import_obsidian.Notice("\u65E0\u6CD5\u6253\u5F00\u63D2\u4EF6\u8BBE\u7F6E\uFF0C\u8BF7\u624B\u52A8\u8FDB\u5165 \u8BBE\u7F6E \u2192 \u7B2C\u4E09\u65B9\u63D2\u4EF6");
  }, 80);
}
...
var USAGE_GUIDE_VERSION = "2026-08-07-jinianri-guide-v15";
...
**\u4E2A\u4EBA\u7248** \u4E0E **\u516C\u7248** \u6807\u8BC6\u4F1A\u663E\u793A\u5728\uFF1A\u7535\u8111\u7AEF\u8BBE\u7F6E\u9875\u6807\u9898\uFF08\u542B\u7248\u672C\u53F7\uFF0C\u5982\u300C\u7EAA\u5FF5\u65E5 \u914D\u7F6E \xB7 \u4E2A\u4EBA\u7248 v3.2.2\u300D\uFF09\u3001\u79FB\u52A8\u7AEF\u5168\u5C4F\u770B\u677F\u9876\u680F\u5FBD\u7AE0\u3002\u516C\u7248\u9700\u6FC0\u6D3B\u540E\u4F7F\u7528\u5168\u90E8\u529F\u80FD\uFF1B\u4E2A\u4EBA\u7248\u5DF2\u5185\u7F6E\u6388\u6743\u3002
...
\u8BBE\u7F6E\u9875\u91C7\u7528 **\u6807\u7B7E\u9875 + \u5361\u7247\u533A\u5757** \u5E03\u5C40\uFF08BrainCore \u98CE\u683C\uFF09\u3002\u672A\u6FC0\u6D3B\u516C\u7248\u65F6\uFF0C\u4EC5 **\u6388\u6743 \xB7 \u6570\u636E \xB7 \u5173\u4E8E** \u53EF\u7528\u3002
...
`;
async function ensureWelcomeGuideFile(app2) {
  try {
    const guideFile = app2.vault.getAbstractFileByPath(USAGE_GUIDE_PATH);
    const bundled = buildGuideFileContent();
    if (!guideFile) {
      await app2.vault.create(USAGE_GUIDE_PATH, bundled);
      return;
    }
    const oldContent = await app2.vault.read(guideFile);
    const fileVersion = extractGuideVersion(oldContent);
    if (!fileVersion) return;
    if (fileVersion === USAGE_GUIDE_VERSION) return;
    await app2.vault.modify(guideFile, bundled);
  } catch (e) {
    console.warn("[\u7EAA\u5FF5\u65E5] \u4F7F\u7528\u6307\u5357\u751F\u6210\u5931\u8D25\uFF1A", e);
  }
}
async function openUsageGuideInNewTab(app2) {
  await ensureWelcomeGuideFile(app2);
  try {
    const file = app2.vault.getAbstractFileByPath(USAGE_GUIDE_PATH);
    if (!file) return;
    const leaf = app2.workspace.getLeaf("tab");
    await leaf.openFile(file);
    app2.workspace.revealLeaf(leaf);
  } catch (e) {
    console.warn("[\u7EAA\u5FF5\u65E5] \u6253\u5F00\u4F7F\u7528\u8BF4\u660E\u5931\u8D25\uFF1A", e);
  }
}
async function openWelcomeGuideOnce(app2, options = {}) {
  const { openImmediately = false, forceOpen = false } = options;
  if (!openImmediately && !forceOpen) return;
  await openUsageGuideInNewTab(app2);
}
...
function getLifeOsVaultKey(app2, suffix) {
  var _a, _b;
  const vaultName = ((_b = (_a = app2.vault) == null ? void 0 : _a.getName) == null ? void 0 : _b.call(_a)) || "UnknownVault";
  return `lifeos:${vaultName}:${suffix}`;
}
function getEnabledLifeOsPlugins(app2) {
  var _a;
  const plugins = ((_a = app2.plugins) == null ? void 0 : _a.plugins) || {};
  return LIFEOS_PLUGIN_CATALOG.filter((p) => {
    const inst = plugins[p.id];
    return inst && inst._loaded !== false;
  });
}
function maybeShowLifeOsSuitePrompt(app2, selfId, selfName) {
  const peers = getEnabledLifeOsPlugins(app2).filter((p) => p.id !== selfId).map((p) => p.name);
  if (peers.length === 0) return;
  const storageKey = getLifeOsVaultKey(app2, "suitePromptSeen");
  try {
    if (localStorage.getItem(storageKey) === "1") return;
  } catch (e) {
  }
  window.setTimeout(() => {
    new import_obsidian2.Notice(`${selfName} \u53EF\u4E0E ${peers.join("\u3001")} \u5E76\u6392\u4F7F\u7528\uFF0C\u6570\u636E\u5747\u4FDD\u5B58\u5728\u540C\u4E00 Obsidian \u5E93\u5185\u3002`, 8e3);
    try {
...
function injectLifeOsActivationStyles() {
  ...
  style.id = id;
  style.textContent = `
.lifeos-act-panel{display:flex;flex:1 1 auto;width:100%;min-height:0;overflow-y:auto;padding:var(--lifeos-sidebar-inset,10px) var(--lifeos-sidebar-inset,10px) 18px!important;box-sizing:border-box;align-items:flex-start;justify-content:center;background:transparent}
.bc-dashboard-view-content.lifeos-act-panel{padding:var(--lifeos-sidebar-inset,10px) var(--lifeos-sidebar-inset,10px) 18px!important;box-sizing:border-box!important}
...
.lifeos-act-status{margin:0;text-align:center;font-size:11px;line-height:1.45;color:var(--lifeos-accent,var(--text-accent));font-weight:600}
...
.lifeos-act-input.lifeos-act-fp{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-weight:700;color:var(--lifeos-accent,var(--text-accent));border-style:dashed;text-align:center}
...
.lifeos-act-btn:active,.lifeos-act-nav-btn:active,.lifeos-act-trial-btn:active,.lifeos-about-link-row button:active{transform:scale(0.97);transition:transform 80ms ease}
...
function renderLifeOsActivationPanel(container, config) {
  ...
  if (config.showTrialButton && config.onTrialStart) {
    ...
    card.createEl("p", { cls: "lifeos-act-philosophy", text: config.firstRunHint });
  }
...
.lifeos-license-status{margin:8px 0 0;font-size:12px;font-weight:600;color:var(--lifeos-accent,var(--text-accent))}
.lifeos-license-trial-hint{margin:0 0 8px;font-size:12px;line-height:1.5;color:var(--lifeos-accent,var(--text-accent))}
...
function renderLifeOsAboutPanel(panel, plugin, options = {}) {
  ...
  LIFEOS_PLUGIN_CATALOG.forEach((item) => {
    ...
    const installed = !!((_b = (_a = plugin.app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b[item.id]);
    if (item.id === selfId) {
      ...
      btn.onclick = () => new import_obsidian2.Notice(`\u8BF7\u5148\u5728 Obsidian \u8BBE\u7F6E \u2192 \u7B2C\u4E09\u65B9\u63D2\u4EF6 \u4E2D\u542F\u7528 ${item.name}`);
    }
  });
}
...
function isObsidianSettingsOpen(app2) {
  try {
    const setting = app2 == null ? void 0 : app2.setting;
...
function runLifeOsUpdateNotice(plugin) {
  ...
  try {
    ...
    try {
      new import_obsidian2.Notice("\u65E0\u6CD5\u6253\u5F00\u66F4\u65B0\u65E5\u5FD7\uFF0C\u8BF7\u91CD\u8BD5\u6216\u91CD\u542F Obsidian");
    } catch (e) {
    }
  }
  return false;
...
function openLifeOsUpdateNoticeFromPlugin(plugin) {
  ...
  if (app2 && isObsidianSettingsOpen(app2)) {
    try {
      app2.setting.close();
    } catch (e) {
    }
    ...
    const poll = () => {
      tries += 1;
      if (!isObsidianSettingsOpen(app2) || tries >= 30) {
        open();
        return;
      }
...
function maybeShowTrialRenewModal(plugin, remainMs, kind) {
  injectLifeOsSharedStyles();
  const modal = new import_obsidian3.Modal(plugin.app);
...
async function startTrialFromActivationPanel(plugin) {
  ...
  new import_obsidian3.Notice(`\u5DF2\u5F00\u59CB ${getTrialHoursLabel()} \u8BD5\u7528`);
  plugin.refreshViews();
}
...
function renderActivationPanel(container, plugin) {
  renderLifeOsActivationPanel(container, {
    ...
    philosophy: typeof PLUGIN_PHILOSOPHY_SUBTITLE === "string" && PLUGIN_PHILOSOPHY_SUBTITLE.trim() ? PLUGIN_PHILOSOPHY_SUBTITLE.trim() : "\u8BB0\u5F55\u751F\u65E5\u3001\u604B\u7231\u3001\u5A5A\u59FB\u7B49\u91CD\u8981\u65E5\u671F\uFF0C\u81EA\u52A8\u8BA1\u7B97\u300C\u5DF2\u8FC7\u65F6\u957F\u300D\u4E0E\u300C\u8DDD\u79BB\u4E0B\u6B21\u8FD8\u6709\u51E0\u5929\u300D\uFF0C\u652F\u6301\u4E09\u6863\u63D0\u9192\u4E0E iCal \u5BFC\u51FA\u3002",
    ...
    firstRunHint: !isTrialEdition() && isLicenseEnforced() && !plugin.settings.licenseActivated ? "\u7B2C\u4E00\u6B65\uFF1A\u590D\u5236\u4E0B\u65B9\u8BBE\u5907\u6307\u7EB9\u53D1\u7ED9\u4F5C\u8005\u83B7\u53D6\u6FC0\u6D3B\u7801 \u2192 \u7C98\u8D34\u6FC0\u6D3B\u7801 \u2192 \u70B9\u300C\u6FC0\u6D3B\u300D\u3002\u6FC0\u6D3B\u540E\u5373\u53EF\u6DFB\u52A0\u7EAA\u5FF5\u65E5\u3002" : void 0,
    ...
    onCopyFingerprint: async (fp) => {
      ...
      new import_obsidian4.Notice(ok ? "\u8BBE\u5907\u6307\u7EB9\u5DF2\u590D\u5236" : "\u8BF7\u624B\u52A8\u5168\u9009\u590D\u5236\u6307\u7EB9");
    },
    onActivate: async (key, msgEl) => {
      ...
      if (plugin.settings.licenseActivated) {
        await plugin.saveSettings();
        new import_obsidian4.Notice("\u6FC0\u6D3B\u6210\u529F\uFF0C\u4E4B\u540E\u5C06\u6C38\u4E45\u6709\u6548");
...
function getSidebarInteractionHint(touchPreferred) {
  const touch = touchPreferred != null ? touchPreferred : import_obsidian5.Platform.isMobile;
...
var DashboardPanel = class {
  constructor(app2, plugin, rootEl) {
    this.bodyEl = null;
  ...
  getVisibleItems() {
    return this.applyViewFilters(this.getBaseVisibleItems(), this.getViewMode());
  }
  ...
  renderEmptyVisible(bodyEl) {
    ...
    renderLifeOsEmptyState(listEl, {
      icon: "\u{1F440}",
      message: "\u6CA1\u6709\u52FE\u9009\u5C55\u793A\u9879\u3002\u5728\u8BBE\u7F6E\u9875\u52FE\u9009\u300C\u5C55\u793A\u300D\uFF0C\u6216\u6DFB\u52A0\u65B0\u4E8B\u9879\u3002",
      ctaLabel: "\u6DFB\u52A0\u7EAA\u5FF5\u65E5",
      onCta: () => this.plugin.openAddEventModal()
    });
  }
  renderFilterEmpty(bodyEl) {
    ...
    renderLifeOsEmptyState(listEl, {
      icon: "\u{1F50E}",
      message: "\u5F53\u524D\u7B5B\u9009\u4E0B\u6CA1\u6709\u7EAA\u5FF5\u65E5\u3002\u8BD5\u8BD5\u5207\u6362\u4E0A\u65B9\u7B5B\u9009\uFF0C\u6216\u6DFB\u52A0\u65B0\u4E8B\u9879\u3002",
      ctaLabel: "+ \u6DFB\u52A0",
      onCta: () => this.plugin.openAddEventModal()
    });
  }
  renderDashboardHeader(bodyEl, items, viewMode) {
    ...
    const filterWrap = filterRow.createDiv({ cls: "jnr-filter-select-wrap jnr-filter-select-compact" });
    ...
    const actions = filterRow.createDiv({ cls: "jnr-toolbar-actions" });
    const switcher = actions.createDiv({ cls: "jnr-view-switch jnr-view-switch-inline" });
    this.renderViewTab(switcher, "list", "\u5217\u8868", viewMode === "list");
    this.renderViewTab(switcher, "timeline", "\u65F6\u95F4\u8F74", viewMode === "timeline");
    this.renderViewTab(switcher, "calendar", "\u65E5\u5386", viewMode === "calendar");
    const addBtn = switcher.createEl("button", {
      text: "\u6DFB\u52A0",
      cls: "jnr-view-tab jnr-header-add-btn",
  ...
  renderViewTab(switcher, mode, label, active) {
    const btn = switcher.createEl("button", {
      text: label,
      cls: `jnr-view-tab${active ? " is-active" : ""}`,
  ...
  renderCalendarView(host, baseItems) {
    ...
    const listTitle = this.plugin.selectedCalendarDay ? `${this.plugin.selectedCalendarDay} \u7684\u7EAA\u5FF5\u65E5` : `${formatMonthTitle(year, month)} \u5185\u7684\u7EAA\u5FF5\u65E5`;
    listHost.createDiv({ cls: "jnr-calendar-list-title", text: listTitle });
    ...
    if (listItems.length === 0) {
      ...
      renderLifeOsEmptyState(emptyHost, {
        icon: "\u{1F4C5}",
        message: this.plugin.selectedCalendarDay ? "\u8FD9\u4E00\u5929\u6CA1\u6709\u7EAA\u5FF5\u65E5" : "\u672C\u6708\u6682\u65E0\u7EAA\u5FF5\u65E5",
        ctaLabel: "+ \u6DFB\u52A0",
        onCta: () => this.plugin.openAddEventModal()
      });
      return;
  ...
  renderSpotlightCard(parent, item) {
    ...
    const open = () => this.plugin.openSettingsForEvent(item.event.id);
  ...
  renderTimelineView(listEl, items) {
    ...
    for (const item of items) {
      ...
      body.createDiv({
        ...
        text: `${calendarTypeLabel(item.event.type)} \xB7 ${this.plugin.getGroupLabel(item.event.group)}`
      });
      const open = () => this.plugin.openSettingsForEvent(item.event.id);
  ...
  updateGroupChevron(header, collapsed) {
    ...
    if (chevron) (0, import_obsidian6.setIcon)(chevron, collapsed ? "chevron-right" : "chevron-down");
  }
  ...
  renderDisplayList(bodyEl, excludeIds) {
    ...
    for (const group of this.plugin.getGroupOrder()) {
      ...
      (0, import_obsidian6.setIcon)(chevron, collapsed ? "chevron-right" : "chevron-down");
  ...
  maybeShowSidebarHintNotice() {
    ...
    new import_obsidian6.Notice(getSidebarInteractionHint(import_obsidian6.Platform.isMobile), 8e3);
  }
  renderFooter(listEl) {
    const foot = listEl.createDiv({ cls: "jnr-sidebar-footer" });
    foot.createDiv({ text: getSidebarInteractionHint(import_obsidian6.Platform.isMobile) });
  ...
  renderCard(parent, item, soonDays, draggable = true) {
    ...
    card.createDiv({
      cls: "jnr-card-meta",
      text: item.isElapsed ? `\u7D2F\u8BA1\u5929\u6570 \xB7 ${formatPassedLabel(item)}` : `${calendarTypeLabel(item.event.type)} \xB7 ${formatPassedLabel(item)}`
    });
    card.addEventListener(draggable ? "jnr-card-click" : "click", () => {
      this.plugin.openSettingsForEvent(item.event.id);
    });
  }
};
...
function tryUnregisterCodeBlockLanguage(app2, language) {
  try {
    const appAny = app2;
...
function tryUnregisterView(app2, type) {
  var _a;
  try {
    app2.workspace.detachLeavesOfType(type);
  } catch (e) {
  }
  try {
    const registry = app2.viewRegistry;
    (_a = registry == null ? void 0 : registry.unregisterView) == null ? void 0 : _a.call(registry, type);
  } catch (e) {
  }
  emptyDashboardDom();
}
function cleanupStaleRegistrations(app2, viewType, codeBlockLang, pluginId = "jinianri") {
  emptyDashboardDom();
  removeStalePluginSettingTabs(app2, pluginId);
  tryUnregisterCodeBlockLanguage(app2, codeBlockLang);
  window.setTimeout(() => {
    try {
      app2.workspace.detachLeavesOfType(viewType);
    } catch (e) {
    }
    emptyDashboardDom();
  }, 0);
}
function safeRegisterView(app2, type, creator, register) {
  try {
...
function removeStalePluginSettingTabs(app2, pluginId) {
  var _a, _b, _c, _d;
  try {
    const tabs = (_a = app2.setting) == null ? void 0 : _a.pluginTabs;
...
var MOBILE_TOP_INSET_PX = 41;
function isMobileAppContext(app2) {
  return app2.isMobile || import_obsidian8.Platform.isMobileApp;
}
...
// src/mobile-dashboard-modal.ts
var MobileDashboardModal = class extends import_obsidian9.Modal {
...
function parseYmd2(ymd) {
  const [y, m, d] = ymd.split("-").map(Number);
  return { year: y || 2e3, month: m || 1, day: d || 1 };
}
...
function clampDay(v) {
  const max = daysInMonth(v.year, v.month);
  return { ...v, day: Math.min(v.day, max) };
}
var DateWheelModal = class extends import_obsidian10.Modal {
  constructor(initial, onConfirm) {
    super(app);
    this.value = clampDay(parseYmd2(initial));
  ...
  yearRange() {
    ...
    const years = [];
    for (let y = now + 5; y >= 1920; y--) years.push(y);
    return years;
  }
  dayRange() {
    const max = daysInMonth(this.value.year, this.value.month);
  ...
  buildWheel(parent, label, items, selected, onSelect) {
    ...
    for (const item of items) {
      const el = list.createDiv({
        cls: "jnr-wheel-item",
        text: String(item),
        attr: { "data-value": String(item) }
      });
      if (item === selected) el.addClass("is-selected");
    }
    list.addEventListener("scroll", () => {
      window.requestAnimationFrame(() => {
        ...
        if (preview) preview.setText(formatYmd2(clampDay(this.value)));
      });
    });
    return list;
  }
  getCenterValue(list) {
    ...
    return closest ? parseInt(closest.getAttribute("data-value"), 10) : null;
  }
  ...
  syncWheels() {
    window.requestAnimationFrame(() => {
      ...
      this.scrollToSelected(this.wheels.month, this.value.month);
  ...
  rebuildDayWheel() {
    ...
    const list = this.wheels.day;
    list.empty();
    for (const d of this.dayRange()) {
      const el = list.createDiv({
        cls: "jnr-wheel-item",
        text: String(d),
        attr: { "data-value": String(d) }
      });
      if (d === this.value.day) el.addClass("is-selected");
    }
    this.scrollToSelected(list, this.value.day);
    const preview = this.contentEl.querySelector(".jnr-date-wheel-preview");
    if (preview) preview.setText(formatYmd2(clampDay(this.value)));
  }
  onClose() {
...
function attachDateWheelButton(btn, getDate, onChange) {
  ...
  btn.addEventListener("click", () => {
    new DateWheelModal(getDate(), (ymd) => {
      onChange(ymd);
      refresh();
    }).open();
  });
...
function showConfirm(app2, options) {
  return new Promise((resolve) => {
    const modal = new ConfirmModal(app2, options, resolve);
    modal.open();
  });
}
var ConfirmModal = class extends import_obsidian11.Modal {
  constructor(app2, options, resolve) {
    super(app2);
    this.settled = false;
...
// src/event-modal.ts
var EventModal = class extends import_obsidian12.Modal {
  constructor(app2, plugin, event, onSubmit, defaultGroup) {
    var _a;
    super(app2);
    ...
    this.event = event != null ? event : {
      ...
      type: "solar",
      group: defaultGroup != null ? defaultGroup : "other",
      sortOrder: 0,
    ...
    this.notesValue = (_a = this.event.notes) != null ? _a : "";
...
function isSampleOnlyData(events) {
  return events.length > 0 && events.every((e) => SAMPLE_EVENT_IDS.has(e.id));
}
var GROUP_BY_NAME = [
...
function getPluginSettingsPath(app2) {
  return (0, import_obsidian13.normalizePath)(`${app2.vault.configDir}/plugins/${PLUGIN_ID}/data.json`);
}
function getPluginEventsPath(app2) {
  return (0, import_obsidian13.normalizePath)(`${app2.vault.configDir}/plugins/${PLUGIN_ID}/events.json`);
}
function getLegacyCombinedDataPath(app2) {
  return getPluginSettingsPath(app2);
}
function getPluginIcsPath(app2) {
  return (0, import_obsidian13.normalizePath)(`${app2.vault.configDir}/plugins/${PLUGIN_ID}/data.ics`);
}
...
var STYLE_ID = "jnr-update-notice-styles";
...
.theme-dark .jnr-update-version:not(.is-latest) .jnr-update-version-head {
  color: #9ed4b8;
  background: rgba(45, 90, 71, 0.18);
}
.theme-dark .jnr-update-version:not(.is-latest) .jnr-update-version-head:hover {
  background: rgba(45, 90, 71, 0.28);
}
...
.is-mobile .jnr-update-modal-mobile.modal {
  ...
  max-height: calc(100dvh - ${MOBILE_TOP_INSET_PX}px - 12px);
  /* \u9876 inset \u53EA\u9760 .jnr-mobile-top-spacer\uFF08\u542B safe-area \u610F\u56FE\uFF09\uFF0C\u52FF\u518D\u53E0 env(safe-area-inset-top) */
  padding-top: 0;
  box-sizing: border-box;
}
.is-mobile .jnr-update-modal-mobile .modal-close-button {
  top: calc(${MOBILE_TOP_INSET_PX}px + 10px);
  z-index: 2;
}
...
function showUpdateNoticeModal(app2, plugin, options) {
  try {
    ...
    injectLifeOsSharedStyles();
    const isMobile = isMobileAppContext(app2);
    const modal = new import_obsidian14.Modal(app2);
    addClasses(modal.modalEl, "jnr-update-modal", isMobile ? "jnr-update-modal-mobile" : "", "lifeos-modal");
    modal.titleEl.hide();
    ...
    if (intro) {
      ...
      addClasses(introEl, "jnr-update-subtitle", "lifeos-philosophy-intro");
    }
    const subEl = hero.createEl("p", { text: PHILOSOPHY_SUBTITLE });
    addClasses(subEl, "jnr-update-subtitle", "lifeos-philosophy-intro");
    ...
    addClasses(btn, "lifeos-modal-primary", "jnr-update-btn");
    btn.onclick = () => {
    ...
    try {
      new import_obsidian14.Notice("\u65E0\u6CD5\u6253\u5F00\u66F4\u65B0\u65E5\u5FD7\uFF0C\u8BF7\u91CD\u8BD5\u6216\u91CD\u542F Obsidian");
    } catch (e) {
    }
  }
}
...
var MOBILE_STYLE_ID = "jnr-settings-mobile-styles-v10";
...
function injectMobileSettingsStyles() {
  ...
  (_a = document.getElementById("jnr-settings-mobile-styles-v1")) == null ? void 0 : _a.remove();
  (_b = document.getElementById("jnr-settings-mobile-styles-v8")) == null ? void 0 : _b.remove();
  (_c = document.getElementById("jnr-settings-mobile-styles-v9")) == null ? void 0 : _c.remove();
  const style = document.createElement("style");
...
.is-mobile .jnr-update-modal-mobile .modal-close-button {
  top: calc(${MOBILE_TOP_INSET_PX}px + 10px) !important;
}
.is-mobile .jnr-update-modal-mobile .modal-content {
  max-height: calc(100dvh - ${MOBILE_TOP_INSET_PX}px - 12px) !important;
}
.is-mobile .jnr-update-modal-mobile .jnr-update-wrap {
  max-height: calc(100dvh - ${MOBILE_TOP_INSET_PX}px - 12px) !important;
}
...
var JinianriSettingTab = class extends import_obsidian15.PluginSettingTab {
  ...
  display() {
    ...
    if (pendingTab && tabDefs.some((t) => t.id === pendingTab)) {
      ...
      if (locked && (pendingTab === "events" || this.plugin.pendingAddEventOnSettings)) {
        new import_obsidian15.Notice("\u8BF7\u5148\u5728\u300C\u6388\u6743\u300D\u5B8C\u6210\u6FC0\u6D3B\uFF0C\u518D\u7BA1\u7406\u7EAA\u5FF5\u4E8B\u9879");
      }
    }
  ...
  renderReminderPanel(panel) {
    ...
    new import_obsidian15.Setting(card).setName("\u7CFB\u7EDF\u901A\u77E5").addToggle(
    ...
    if (import_obsidian15.Platform.isMobile) {
      card.createEl("p", {
        ...
        text: "\u79FB\u52A8\u7AEF\u6682\u4E0D\u652F\u6301\u7CFB\u7EDF\u901A\u77E5\uFF0C\u8BF7\u4F7F\u7528 Obsidian \u5185\u5F39\u7A97\u3002"
      });
    } else if (import_obsidian15.Platform.isMacOS) {
      card.createEl("p", {
        cls: "jnr-settings-section-hint",
    ...
    new import_obsidian15.Setting(card).setName("\u6BCF\u65E5\u65F6\u95F4").addText((text) => {
      ...
      text.onChange(async (value) => {
        this.plugin.setReminderTime(value);
        this.plugin.settings.lastReminderCheckDate = "";
        await this.plugin.persistSettings();
      });
    });
    new import_obsidian15.Setting(card).setName("\u6D4B\u8BD5\u63D0\u9192").addButton(
  ...
  renderEventsPanel(panel) {
    ...
    const card = this.createBlock(
      grid,
      "\u7EAA\u5FF5\u4E8B\u9879",
      "\u8868\u683C\u5185\u53EF\u5FEB\u901F\u6539 emoji + \u540D\u79F0 / \u65E5\u671F / \u5F00\u5173\uFF1B\u70B9 \u270F\uFE0F \u6253\u5F00\u5B8C\u6574\u7F16\u8F91\uFF08\u5907\u6CE8\u3001\u5206\u7EC4\u7B49\uFF09\u3002\u4FA7\u8FB9\u680F\u5217\u8868\u89C6\u56FE\u7528\u4E8E\u6D4F\u89C8\u3001\u6298\u53E0\u5206\u7EC4\u4E0E\u62D6\u52A8\u6392\u5E8F\uFF1B\u6708\u5386 / \u65F6\u95F4\u8F74\u4E3A\u53EA\u8BFB\u6D4F\u89C8\u3002"
    );
    ...
    for (const groupId of this.plugin.getGroupOrder()) {
      ...
      const nameInput = groupHead.createEl("input", {
        ...
        attr: { placeholder: "\u5206\u7EC4\u540D\u79F0", "aria-label": "\u5206\u7EC4\u540D\u79F0" }
      });
      ...
      const events = this.plugin.events.filter((e) => e.group === groupId).sort((a, b) => a.sortOrder - b.sortOrder);
      if (events.length === 0) {
        const emptyHost = block.createDiv({ cls: "jnr-settings-group-empty" });
        renderLifeOsEmptyState(emptyHost, {
          icon: "\u{1F4DD}",
          message: "\u6682\u65E0\u4E8B\u9879\uFF0C\u70B9\u51FB\u4E0B\u65B9\u6309\u94AE\u6216\u5206\u7EC4\u300C+ \u4E8B\u9879\u300D\u6DFB\u52A0",
          ctaLabel: "+ \u4E8B\u9879",
  ...
  renderGeneralPanel(panel) {
    ...
    card.createEl("p", {
      cls: "jnr-settings-section-hint",
      text: "\u770B\u677F\u652F\u6301\u5217\u8868 / \u65F6\u95F4\u8F74 / \u6708\u5386\u4E09\u79CD\u89C6\u56FE\u3002\u6298\u53E0\u5206\u7EC4\u3001\u62D6\u52A8\u6392\u5E8F\u4E0E\u5E95\u90E8\u4EA4\u4E92\u63D0\u793A\u4EC5\u5728\u5217\u8868\u89C6\u56FE\uFF1B\u9876\u90E8\u7B5B\u9009\u4E0B\u62C9\u9002\u7528\u4E8E\u5168\u90E8\u89C6\u56FE\u3002"
    });
  ...
  renderDataPanel(panel) {
    const grid = panel.createDiv({ cls: "jnr-settings-grid" });
    const card = this.createBlock(grid, "\u6570\u636E", "\u66F4\u65B0\u65E5\u5FD7\u4E0E\u63D2\u4EF6\u6570\u636E\u6587\u4EF6\u8DEF\u5F84\u3002");
    new import_obsidian15.Setting(card).setName("\u66F4\u65B0\u65E5\u5FD7").addButton(
      (btn) => btn.setButtonText("\u67E5\u770B").onClick(() => this.plugin.showUpdateNotice(true))
    );
    card.createEl("p", {
      cls: "jnr-settings-section-hint",
      text: isLicenseEnforced() ? `\u5F53\u524D\u7248\u672C ${getPluginVersionLabel()}\u3002\u66F4\u65B0\u65E5\u5FD7\u4E0E\u4F7F\u7528\u8BF4\u660E\u4E0D\u4F1A\u81EA\u52A8\u5F39\u51FA\uFF0C\u8BF7\u624B\u52A8\u5728\u6FC0\u6D3B\u9875\u6216\u8BBE\u7F6E\u9875\u6253\u5F00\u3002\u672A\u6FC0\u6D3B\u516C\u7248\u65F6\uFF0C\u8BF7\u5728\u300C\u6388\u6743\u300D\u9875\u5B8C\u6210\u6FC0\u6D3B\u3002` : `\u5F53\u524D\u7248\u672C ${getPluginVersionLabel()}\u3002\u66F4\u65B0\u65E5\u5FD7\u4E0E\u4F7F\u7528\u8BF4\u660E\u4E0D\u4F1A\u81EA\u52A8\u5F39\u51FA\uFF0C\u8BF7\u624B\u52A8\u5728\u6FC0\u6D3B\u9875\u6216\u8BBE\u7F6E\u9875\u6253\u5F00\u3002`
    });
    ...
    new import_obsidian15.Setting(card).setName("data.json").addButton(
...
${settingsPath}`
    ...
    new import_obsidian15.Setting(card).setName("events.json").addButton(
...
\u6B64\u64CD\u4F5C\u4E0D\u53EF\u64A4\u9500\uFF0C\u662F\u5426\u7EE7\u7EED\uFF1F`;
  ...
  renderEventRow(list, event, index) {
    ...
    const iconInput = nameCell.createEl("input", {
      cls: "jnr-event-icon-input",
    ...
    modeSelect.addEventListener("change", () => {
      ...
      if (event.recurrence === "elapsed") event.type = "solar";
      syncMode();
      this.plugin.scheduleSaveEvents();
    });
    typeSelect.addEventListener("change", () => {
...
var ReminderAlertModal = class extends import_obsidian16.Modal {
  constructor(app2, title, body, onDismiss) {
    super(app2);
  ...
  onOpen() {
    ...
    contentEl.createEl("h2", { cls: "jnr-reminder-alert-title", text: this.titleText });
    contentEl.createEl("p", {
      cls: "jnr-reminder-alert-hint",
      text: "\u8BF7\u70B9\u51FB\u300C\u77E5\u9053\u4E86\u300D\u786E\u8BA4\uFF0C\u4EE5\u514D\u91CD\u590D\u63D0\u9192"
    });
    const bodyEl = contentEl.createDiv({ cls: "jnr-reminder-alert-body" });
    ...
    this.blockBackdropDismiss();
  }
  onClose() {
    var _a;
    (_a = this.onDismiss) == null ? void 0 : _a.call(this);
  }
  /** 禁止点击遮罩或 Esc 关闭，必须点按钮 */
  blockBackdropDismiss() {
    const container = this.modalEl.closest(".modal-container");
    const bg = container == null ? void 0 : container.querySelector(".modal-bg");
    const stopBg = (e) => {
      e.stopImmediatePropagation();
      e.preventDefault();
    };
    bg == null ? void 0 : bg.addEventListener("click", stopBg, true);
    this.register(() => bg == null ? void 0 : bg.removeEventListener("click", stopBg, true));
    const stopEsc = (e) => {
      if (e.key === "Escape") {
        e.stopImmediatePropagation();
        e.preventDefault();
    ...
    this.register(() => window.removeEventListener("keydown", stopEsc, true));
  }
};
var ReminderModalQueue = class {
  ...
  enqueue(app2, title, body, onDismiss) {
    this.app = app2;
    this.queue.push({ title, body, onDismiss });
    this.drain();
  }
  dismissAll() {
    var _a;
  ...
  drain() {
    ...
    this.showing = true;
    const { title, body, onDismiss } = this.queue.shift();
    const modal = new ReminderAlertModal(this.app, title, body, () => {
...
function showBlockingReminderModal(app2, title, body, onDismiss) {
  modalQueue.enqueue(app2, title, body, onDismiss);
}

// src/notification-utils.ts
function showDesktopNotification(title, body) {
  if (!import_obsidian17.Platform.isDesktopApp) return;
  try {
...
function showReminderNotice(title, body, options = {}) {
  const { app: app2, obsidian = true, desktop = false, onDismiss } = options;
  let shown = false;
  if (obsidian && app2) {
    showBlockingReminderModal(app2, title, body, onDismiss);
    shown = true;
  }
  if (desktop && import_obsidian17.Platform.isDesktopApp) {
    showDesktopNotification(title, body);
    if (!obsidian) onDismiss == null ? void 0 : onDismiss();
    shown = true;
  }
  if (!shown) {
    new import_obsidian17.Notice("\u8BF7\u5148\u5F00\u542F\u300CObsidian \u5185\u5F39\u7A97\u300D\u6216\u300C\u684C\u9762\u7CFB\u7EDF\u901A\u77E5\u300D", 5e3);
  }
  return shown;
}
...
var ReminderService = class {
  constructor(plugin) {
    ...
    /** 同一次 Obsidian 会话内只自动检查一轮，避免每分钟重复弹窗 */
  ...
  checkReminders(force = false) {
    ...
    const notify = (title, body, key) => {
      const { settings: settings2 } = this.plugin;
      const hasChannel = settings2.enableObsidianNotice || settings2.enableDesktopNotification && import_obsidian19.Platform.isDesktopApp;
      if (!hasChannel) return;
      showReminderNotice(title, body, {
        app: this.plugin.app,
        obsidian: settings2.enableObsidianNotice,
        desktop: settings2.enableDesktopNotification,
        onDismiss: () => void this.markReminderDismissed(key)
      });
    };
  ...
  /** 用户点「知道了」后写入 data.json，同一条提醒不再重复弹出 */
  async markReminderDismissed(key) {
    const { settings } = this.plugin;
    if (!settings.sentReminderKeys.includes(key)) {
      settings.sentReminderKeys = [...settings.sentReminderKeys, key].slice(-500);
    }
    settings.lastReminderCheckDate = formatYmd(/* @__PURE__ */ new Date());
    try {
      await this.plugin.saveSettings();
    } catch (err) {
      console.error("[\u7EAA\u5FF5\u65E5] \u63D0\u9192\u5DF2\u8BFB\u72B6\u6001\u4FDD\u5B58\u5931\u8D25", err);
    }
  }
  pruneSentKeys() {
  ...
  async readEventsFile(path) {
    ...
    if (file instanceof import_obsidian20.TFile) {
      try {
        ...
        return parsed.events.map((e) => normalizeEvent(e));
      } catch (e) {
    ...
    if (await adapter.exists(path)) {
      try {
        ...
        new import_obsidian20.Notice("\u7EAA\u5FF5\u65E5\u6570\u636E\u8BFB\u53D6\u5931\u8D25\uFF0C\u5C06\u5C1D\u8BD5\u6062\u590D\u9ED8\u8BA4\u6570\u636E");
      }
    }
    return null;
  }
...
async function exportIcsToVault(app2, events, getComputed, _settings, icsPath) {
  const path = (0, import_obsidian22.normalizePath)(icsPath != null ? icsPath : getPluginIcsPath(app2));
  const content = buildIcsContent(events, getComputed, _settings);
  const folder = path.split("/").slice(0, -1).join("/");
  if (folder) await app2.vault.adapter.mkdir((0, import_obsidian22.normalizePath)(folder)).catch(() => {
  });
  const existing = app2.vault.getAbstractFileByPath(path);
  if (existing instanceof import_obsidian22.TFile) {
    await app2.vault.modify(existing, content);
    return existing;
  }
  ...
  try {
    return await app2.vault.create(path, content);
  } catch (e) {
    await adapter.write(path, content);
    return { path };
  }
...
var JinianriPlugin = class extends import_obsidian23.Plugin {
  constructor() {
    super(...arguments);
  ...
  async bootstrap() {
    ...
    try {
      try {
        (0, import_obsidian23.addIcon)("calendar-heart", CALENDAR_HEART_ICON);
      } catch (err) {
      ...
      safeAddRibbonIcon(this, "calendar-heart", "\u6253\u5F00\u7EAA\u5FF5\u65E5", () => {
      ...
      this.codeBlockProcessor = safeRegisterCodeBlockProcessor(
        this,
        "jinianri",
        (el) => this.renderInlineDashboard(el)
      );
      this.settingTab = new JinianriSettingTab(this.app, this);
      safeAddSettingTab(this, this.settingTab);
      if (!this.alive) return;
      this.registerInterval(() => this.tickDayRefresh(), DAY_REFRESH_INTERVAL_MS);
      ...
      new import_obsidian23.Notice(`\u7EAA\u5FF5\u65E5\u63D2\u4EF6\u52A0\u8F7D\u5931\u8D25\uFF1A${err instanceof Error ? err.message : String(err)}`);
    } finally {
      this.bootstrapping = false;
    }
  }
  onunload() {
    ...
    window.setTimeout(() => removeStalePluginSettingTabs(app2, pluginId), 200);
  }
  async loadSettings() {
    var _a;
  ...
  refreshInlineBlocks() {
    for (const el of [...this.inlineHosts]) {
      if (el.isConnected) this.renderInlineDashboard(el);
  ...
  onLicenseActivated() {
    ...
    if (this.events.length === 0) {
      new import_obsidian23.Notice("\u6FC0\u6D3B\u6210\u529F\uFF01\u5728\u4FA7\u8FB9\u680F\u70B9\u300C\u6DFB\u52A0\u7B2C\u4E00\u4E2A\u7EAA\u5FF5\u65E5\u300D\uFF0C\u6216\u8FDB\u5165\u8BBE\u7F6E\u6DFB\u52A0");
    }
  }
  ...
  openAddEventModal() {
    if (!this.requireLicense()) return;
    new EventModal(this.app, this, null, async (result, updated) => {
      if (result.deleted) {
        const idx = this.events.findIndex((e) => e.id === updated.id);
        if (idx >= 0) this.events.splice(idx, 1);
      } else {
        updated.sortOrder = this.events.filter((e) => e.group === updated.group).length;
  ...
  requireLicense() {
    ...
    if (isTrialEdition() && !this.settings.trialWelcomeSeen) {
      new import_obsidian23.Notice("\u8BF7\u5148\u70B9\u51FB\u300C\u5F00\u59CB\u8BD5\u7528\u300D\u6216\u8F93\u5165\u6FC0\u6D3B\u7801");
    } else if (isTrialEdition()) {
      new import_obsidian23.Notice("\u8BD5\u7528\u5DF2\u5230\u671F\uFF0C\u8BF7\u6253\u5F00\u7EAA\u5FF5\u65E5\u9762\u677F\u8F93\u5165\u6FC0\u6D3B\u7801");
    } else {
      new import_obsidian23.Notice("\u8BF7\u6253\u5F00\u7EAA\u5FF5\u65E5\u9762\u677F\uFF08\u65E5\u5386\u56FE\u6807\uFF09\u6216\u8BBE\u7F6E\u5B8C\u6210\u6FC0\u6D3B");
    }
    return false;
  }
  ...
  async exportIcal() {
    ...
    try {
      ...
      if (!import_obsidian23.Platform.isMobile && file instanceof import_obsidian23.TFile) {
        await this.app.workspace.getLeaf().openFile(file);
      }
    } catch (err) {
      new import_obsidian23.Notice(`iCal \u5BFC\u51FA\u5931\u8D25\uFF1A${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  async activateSidebar(options) {
    ...
    try {
      ...
      new import_obsidian23.Notice("\u65E0\u6CD5\u6253\u5F00\u7EAA\u5FF5\u65E5\u4FA7\u8FB9\u680F");
    }
  }
  openMobileDashboard() {
    var _a;
  ...
  renderInlineDashboard(container) {
    ...
    const items = computeAll(this.events.filter((e) => e.showInSidebar));
    if (items.length === 0) {
      ...
      container.createEl("button", { text: "+ \u6DFB\u52A0", cls: "mod-cta jnr-inline-empty-cta" }).addEventListener("click", () => this.openAddEventModal());
      return;
    }
    const tableWrap = container.createDiv({ cls: "jnr-table-wrap" });
    const compact = import_obsidian23.Platform.isMobile || container.clientWidth < 480;
    if (compact) {
      ...
      if (items.length > 12) {
        cards.createEl("p", {
          cls: "jnr-inline-empty-hint",
          text: `\u8FD8\u6709 ${items.length - 12} \u9879\uFF0C\u8BF7\u6253\u5F00\u4FA7\u8FB9\u680F\u67E5\u770B\u5168\u90E8`
        });
      }
      return;
    }
    ...
    ["\u7EAA\u5FF5\u4E8B\u9879", "\u539F\u59CB\u65E5\u671F", "\u7C7B\u578B", "\u7D2F\u8BA1\u65F6\u957F", "\u4E0B\u6B21\u65E5\u671F (\u9633)", "\u8DDD\u79BB\u4E0B\u6B21"].forEach(
